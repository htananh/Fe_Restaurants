...
//     await axios
//       .delete(`${process.env.NEXT_PUBLIC_BASE_URL}/foods/${item.id_mon_an}`, {
...
//     await axios
//       .put(
//         `${process.env.NEXT_PUBLIC_BASE_URL}/foods/${item.id_mon_an}`,
//         {
//           tenMon,
//           loai,
//           donGia,
...
const AdminFoodList = ({ item }) => {
  const [openModal, setOpenModal] = useState(false);
  const [tenMon, setName] = useState(item.tenMon);
  const [loai, setCategory] = useState(item.loai);
  const [donGia, setCost] = useState(item.donGia);
  ...
  const handleDelete = async () => {
    try {
      ...
      await axios.delete(`${process.env.NEXT_PUBLIC_BASE_URL}/foods/${item.id_mon_an}`, {
        headers: { Authorization: token },
      });
