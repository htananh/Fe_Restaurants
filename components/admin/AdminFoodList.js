...
const AdminFoodList = ({ item }) => {
  ...
  const handleDelete = async () => {
    ...
    if (!item.idMonAn) {
      enqueueSnackbar("Food ID is missing", { variant: "error" });
      return;
    }
    await axios
      .delete(`${process.env.NEXT_PUBLIC_BASE_URL}/foods/${item.idMonAn}`, {
  ...
  const handleUpdate = async (e) => {
    ...
    await axios
      .put(
        `${process.env.NEXT_PUBLIC_BASE_URL}/foods/${item.idMonAn}`,
        {
          tenMon,
          loai,
          donGia,
